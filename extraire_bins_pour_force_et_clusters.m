function binnedData = extraire_bins_pour_force_et_clusters(datatable,fs,type,type2,varargin)
%
...
time2success_i=find(strcmpi('time2succes',row_names));

if strfind(type,'mu')
    a=strfind(row_names,'_mu');
    tf=cellfun('isempty',a);a(tf)={0};
    spike_i=find(logical(cell2mat(a)));
elseif strfind(type,'su')
    a=strfind(row_names,'_su');
    tf=cellfun('isempty',a);a(tf)={0};
    spike_i=find(logical(cell2mat(a)));
    %         b=strfind(row_names,'ch8');
    %         tf=cellfun('isempty',b);b(tf)={0};
    %         c=strfind(row_names,'ch24');
    %         tf=cellfun('isempty',c);c(tf)={0};
    %         spike_i=union(intersect(find(logical(cell2mat(a))),find(logical(cell2mat(b)))),...
    %             intersect(find(logical(cell2mat(a))),find(logical(cell2mat(c)))));
elseif strfind(type,'tout')
    spike_i  = find(strncmpi('ch',row_names,2));
end

jackpot_match = strfind(row_names,'trial_type');
successful_match = strfind(row_names,'successful');
jackpot_i = [];
successful_i = [];
for i = 1:num_rows
...
for i = 1:num_rows
    if ~isempty(successful_match{i})
        successful_i = [successful_i; i];
    end
end
...
successful_bin = cell(num_trials,1);
    ...
    for trial = 1:num_trials
        ...
        if isempty(force_i)
            ...
            for F = 1:num_force
                ...
                force_bin{trial,F} = double(interp1(forcetimebins, tempForce, timeframe{trial},'linear','extrap')');
                %             %Normalize Force
                %             if params.NormData
        ...
        if isempty(spike_i)
            ...
            binnedData=[]; %il arrive qu'il n'y ait aucun cluster (su par exemple)
            return
        else
            for S = 1:num_spikes
        ...
        %% 6-Bin successful
        if isempty(successful_i)
            disp('No successful info was found');
        else
            successful_bin{trial} = datatable{successful_i(1),trial}{:};
        end
        
        %% Time2Success
        
        time2success(trial,1)=cell2mat(datatable{time2success_i,trial})';
        
    end
    
elseif strfind(type2,'reward')
    ...
    for trial = 1:num_trials
        ...
        if isempty(force_i)
            ...
            forcetimebins = (0:numel(datatable{force_i(1),trial}{:})-1)/force_fs;
            for F = 1:num_force
                tempForce = fliplr(datatable{force_i(F),trial}{:});
                force_bin{trial,F} = flipud(double(interp1(forcetimebins, tempForce, timeframe{trial},'linear','extrap')'));
                %             %Normalize Force
                %             if params.NormData
                %                 %dont use the max because of possible outliars, use 99% percentile
                %                 forceNormRatio     = prctile(force_bin{trial,F},99);
        ...
        if isempty(spike_i)
            ...
            for S = 1:num_spikes
                ts = (floor(trial_dur{trial}*spike_fs)-flipud(datatable{spike_i(S),trial}{:})+1)/spike_fs;
                spike_bin{trial,S} = flipud(train2bins(ts,timeframe{trial})');
            end
        end
...
successful_bin=cell2mat(successful_bin);

%% Outputs
% binnedData = table(jackpot_bin, force_bin, emg_bin, LFP_bin, beta_bin, lgamma_bin, hgamma_bin, vhgamma_bin, spike_bin);
binnedData = table(jackpot_bin, force_bin, spike_bin, successful_bin,time2success);
binnedData.jackpot_bin=cell2mat(binnedData.jackpot_bin);
% if strcmp(type4,'pasmoyennage')
%     binnedData.spike_bin(size(binnedData.spike_bin,1)+1,:)=row_names(spike_i);
% end
